  ...
  const form = useForm({
    ...
    onSubmit: async ({ value, formApi }) => {
      const created = await withToast(
        addCategory({ userId, name: value.name, type: value.type }),
    ...
    <main className="ui-page">
      ...
      <section className="ui-card mb-4 p-5">
        ...
        <p className="ui-muted mb-4 text-xs">
          The built-in categories are always available. Anything you add here
          shows up in the card editor and in the assistant’s suggestions.
        </p>
            ...
            {(field) => (
              <div className="flex min-w-[12rem] flex-1 flex-col gap-1">
                ...
                {field.state.meta.errors.length > 0 ? (
                  <span className="text-xs text-[#b4462f]">
                    {String(
                ...
                <ul className="flex flex-wrap gap-1.5">
                  {categories[type].map((name) => {
                    ...
                    return (
                      <li
                        key={name}
                        className="ui-pill inline-flex items-center gap-1.5"
                      >
                        {name}
                        {custom ? (
                          <button
      ...
      <section className="ui-card p-5">
        ...
        <p className="ui-muted mt-1 text-xs">
          Cards, categories and settings are stored per account. Removing a
          custom category leaves existing cards untouched.
        </p>
      </section>
    </main>
  )
